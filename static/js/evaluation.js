...
$(document).on("click", "span.feedback", function(e) {
			...
			startOffset = para.indexOf(txt);
			endOffset = startOffset + txt.length;
			part1 = para.substring(0, startOffset);
			part2 = para.substring(endOffset, para.length);
			sentenceStartOffset = part1.lastIndexOf(".");
			sentenceEndOffset = part2.indexOf(".");
			var sentence = part1.substring(sentenceStartOffset) + "<head>" + txt +"</head>" + part2.substring(0, sentenceEndOffset) + ".";
			...
			querySalsa(txt, sentence, example);
...
function querySalsa(word, sentence, example){
  $.ajax({
		url: "http://lit.eecs.umich.edu/~mazab/salsa_efl/static/mycgi",
		type: "GET",
		datatype:"json",
		...
		success: function(response){
            ...
            $('#hd_example').hide();
			$('#hd_example').html("");
			$('#hd_synList').append('<div id="headword"><h3>Synonyms for ' + word + ': </h3></div>');
  ...
  function displaySynonyms (synList) {
		...
		if(syns.length < 2)
			 $('#hd_synList').append('<div class="synonym">' + 'Could not find a synonym for this word' + '</div>');
		for (var i=0; i < syns.length && i < 10; i++) {
        //alert(syns[i]);
  ...
  function displayExample (word, exampleSen) {
    $('#hd_example').append('<h4>Example Sentence:</h4>' + exampleSen);
	$('#hd_example').append('<div class="hd_useful">Did you find this answer useful?</div>');
	$('#hd_example').append('<div class="hd_yes" onclick="rateAnswer(\'' + word + '\',\'1\')">Yes</div>');
	$('#hd_example').append('<div class="hd_no" onclick="rateAnswer(\'' + word + '\',\'-1\')">No</div>');
   ...
   $('#not_useful').on('click', function(e) {
        alert("Useful");
        });
  ...
  function rateAnswer (word, rate) {
		wordUsefulness[word] = rate;
		console.log(wordUsefulness);
    }
